  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button variant="primary" onClick={() => setUploadModalOpen(true)} className="gap-2">
      ...
      <Card className="p-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <Button variant={filterCategory === 'all' ? 'primary' : 'ghost'} size="sm" onClick={() => setFilterCategory('all')}>Tous</Button>
            {PMO_CATEGORIES.map(cat => (
              <Button key={cat.id} variant={filterCategory === cat.id ? 'primary' : 'ghost'} size="sm" onClick={() => setFilterCategory(cat.id)}>
      ...
      {filteredDocs.length === 0 ? (
        <EmptyState icon={FileText} title="Aucun document" description="Commencez par uploader votre premier document" action={<Button variant="primary" onClick={() => setUploadModalOpen(true)}><Upload size={18} />Uploader un document</Button>} />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredDocs.map(doc => {
            ...
            return (
              <motion.div key={doc.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }}>
                <Card className="p-4 hover:border-brand-gold-500/50 transition-all group">
                  <div className="flex items-start gap-3">
                    <div className={cn("w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0", `bg-${category?.color}-500/20`)}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        <Badge variant={category?.color} className="text-xs">{doc.categoryLabel}</Badge>
                        <span className="text-xs text-gray-500">{formatFileSize(doc.size)}</span>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-gray-400">
                        <User size={12} /><span>{doc.uploadedBy}</span><span>•</span><span>{formatDate(doc.uploadedAt)}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="ghost" size="sm" onClick={() => handleDownload(doc)} className="flex-1">
      ...
      <Modal isOpen={uploadModalOpen} onClose={() => { setUploadModalOpen(false); setUploadProgress(0); }} title="Uploader un document">
          ...
          {uploadProgress > 0 && uploadProgress < 100 && (
            <div>
              ...
              <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                <motion.div className="h-full bg-brand-gold-500" initial={{ width: 0 }} animate={{ width: `${uploadProgress}%` }} />
              </div>
            </div>
          )}
        </div>
      ...
      <Modal isOpen={categoryModalOpen} onClose={() => { setCategoryModalOpen(false); setSelectedFile(null); }} title="Catégoriser le document" size="lg">
        <div className="space-y-4">
          <p className="text-gray-400">Fichier: <span className="text-white font-semibold">{selectedFile?.name}</span></p>
          <p className="text-sm text-gray-500">Sélectionnez la catégorie PMO appropriée pour ce document:</p>
          <div className="grid grid-cols-2 gap-3">
            {PMO_CATEGORIES.map(category => (
              <button key={category.id} onClick={() => handleCategorySelect(category)} className={cn("p-4 rounded-lg border-2 transition-all text-left", "hover:border-brand-gold-500 hover:bg-brand-gold-500/10", "border-gray-700 bg-gray-800/50")}>
